...
KE.setting = {
	...
	afterTab : function(id) {
	    if(typeof KE.util.afterTab == "undefined"){
...
KE.selection = function(doc) {
	...
	this.addRange = function(keRange) {
		if ('object' != typeof ebagShell && KE.browser.GECKO && KE.browser.VERSION < 3) return;
		this.keRange = keRange;
		if (KE.browser.IE) {
...
KE.cmd = function(id) {
	...
	this.remove = function(tags , expTages) {//expTages (排除标签)
		var self = this;
		var keRange = this.keRange;
		var startNode = keRange.startNode;
		var startPos = keRange.startPos;
		...
		for (var i = 0, length = nodeList.length; i < length; i++) {
			...
			if (node.nodeType == 1) {
				...
				if(typeof expTages == 'object'&& $.inArray(tagName , expTages) >= 0)continue;
...
KE.util = {
	...
	trim : function(str) {
        var cnspac = "　　";
        var hascnspac = false;
        if(str.substring(0, cnspac.length) == cnspac) {
            hascnspac = true;
        }
		str = str.replace(/^\s+|\s+$/g, "");
        if(hascnspac) str = cnspac + str;
        return str;
	},
	...
	resize : function(id, width, height, isCheck, isResizeWidth) {
		...
		g.height = parseInt(height);
		var diff = g.height - g.toolbarHeight - g.statusbarHeight;
		if (diff >= 0) {
	...
	setDefaultPlugin : function(id) {
		...
		for (var i = 0; i < items.length; i++) {
			...
			plugin.click = (function(item) {
				return function(id) {
				    KE.onDefaultPluginClick();
                    KE.util.execCommand(id, item, null);
					KE.afterDefaultPluginClick();
                    if(KE.g[id].onChanged) {
                        KE.g[id].onChanged();
                    }
				};
			})(item);
...
KE.menu = function(arg){
	function getPos(width, height) {
		...
		if (width > 0 || height > 0) {
			...
			var maxLeft = scrollPos.x + KE.g[id].iframe.clientWidth + iframePos.x - width - 20;
			if (x > maxLeft) x = maxLeft;
			var maxTop = KE.g[id].iframe.clientHeight + iframePos.y - height - 5;
	...
	function init() {
		...
		var div = KE.$$('div');
		div.className = 'ke-' + this.type + (typeof arg.class_name != 'undefined' ? ' '+ arg.class_name : '');
	...
	this.add = function(html, event, options) {
		...
		var self = this;
		var cDiv = KE.$$('div');
		var titalname = typeof options != 'undefined' && typeof options.titlename != 'undefined' ? ' '+ options.titlename  :  '';
		cDiv.className = 'ke-' + self.type + '-item' + titalname;
		...
		cDiv.onmouseover = function() {
			this.className = 'ke-' + self.type + '-item ke-' + self.type + '-item-on' + titalname;
			center.className = 'ke-' + self.type + '-center ke-' + self.type + '-center-on';
		};
		cDiv.onmouseout = function() {
			this.className = 'ke-' + self.type + '-item' + titalname;
			center.className = 'ke-' + self.type + '-center';
		};
		cDiv.onclick = event;
...
KE.dialog = function(arg){
	...
	this.show = function() {
        ...
        if(typeof arg.isDialog == 'undefined' || arg.isDialog) {
...
KE.toolbar = {
	updateState : function(id) {
		var cmdList = [
			...
			'bold', 'italic', 'underline', 'strikethrough','imagenone','imageleft','imageright','imagecenter','imagefloat'
		];
		for (var i = 0; i < cmdList.length; i++) {
			var cmd = cmdList[i];
			if(this.isDisabled(id , cmd))continue;
			var state = false;
			try {
			    switch(cmd){
				   case 'imagefloat' : //图片浮动状态
				       state = KE.plugin.imagefloat.queryCommandState(id);
					   break;
				   default:
		...
		var attach = ['fontname','fontsize','myfontsize'];
		var g = KE.g[id];
		if(typeof g.keRange != 'undefined'){
			var startNode = g.keRange.startNode;
			var p = g.keRange.getParentElement();
			for (var i = 0; i < attach.length ; i++){
				KE.plugin[attach[i]].select(id , p);
			}
		}
	},
	...
	isDisabled : function(id , cmd){
	    var result = false;
	    if (KE.g[id].toolbarIcon[cmd]) {
		     var a = KE.g[id].toolbarIcon[cmd][0];
			 if(a.className.indexOf('ke-icon-disabled') > -1)result = true;
		}
		return result;
	},
	select : function(id, cmd) {
		if (KE.g[id].toolbarIcon[cmd]) {
			var a = KE.g[id].toolbarIcon[cmd][0];
			a.className = 'ke-icon ke-icon-selected ke-'+cmd+'-icon';
			a.onmouseover = null;
			a.onmouseout = null;
		}
	},
	unselect : function(id, cmd) {
		if (KE.g[id].toolbarIcon[cmd]) {
			var a = KE.g[id].toolbarIcon[cmd][0];
			a.className = 'ke-icon ke-'+cmd+'-icon';
			a.onmouseover = function(){ this.className = 'ke-icon ke-icon-on ke-'+cmd+'-icon'; };
			a.onmouseout = function(){ this.className = 'ke-icon ke-'+cmd+'-icon'; };
		}
	},
	...
	ableCmd : function(id , cmd){
	    var obj = KE.g[id].toolbarIcon[cmd];
		if(typeof obj == 'undefined')return;
        var a = obj[0];
		var span = obj[1];
		this._setAttr(id, a, cmd);
		KE.util.setOpacity(span, 100);
	},
	disable : function(id, arr) {
		KE.each(KE.g[id].toolbarIcon, function(cmd, obj) {
			if (!KE.util.inArray(cmd, arr)) {
				var a = obj[0];
				var span = obj[1];
				a.className = 'ke-icon ke-'+ cmd +'-icon ke-icon-disabled';
	...
	create : function(id) {
		...
		for (var i = 0; i < length; i++) {
            ...
            //showButtonText
            //todo::addylee
            if(KE.g[id].showButtonText && KE.buttons && typeof(KE.buttons[cmd]) != 'undefined') {
                var label = KE.$$('label');
                label.innerHTML = KE.buttons[cmd];
...
KE.history = {
	addStackData : function(stack, data , callback) {
		var prev = '';
		if (stack.length > 0) {
	...
	removeStachData : function(id){
	    var g = KE.g[id];
	    var num = g.undoStack.length;
		if(num < 2)return;
		g.undoStack.splice(num - 1 , 1);
	},
	add : function(id, minChangeSize) {
		...
		this.addStackData(g.undoStack, html , function(){
		    if(g.undoStack.length > 0)KE.toolbar.ableCmd(id, 'undo');
		});
	},
	undo : function(id) {
		var g = KE.g[id];

		if (g.undoStack.length <= 1) {
		   KE.toolbar.disableCmd(id, 'undo');
		   return;
		}
		KE.toolbar.ableCmd(id, 'redo');
		var html = KE.util.getSrcData(id);
		...
		if(g.undoStack.length == 0)KE.toolbar.disableCmd(id, 'undo');
	},
	redo : function(id) {
		var g = KE.g[id];
		if (g.redoStack.length == 0){
		    KE.toolbar.disableCmd(id, 'redo');
		    return;
		}
		KE.toolbar.ableCmd(id, 'undo');
		var html = KE.util.getSrcData(id);
...
KE.create = function(id, mode) {
	...
	KE.onchange(id, function(id) {
		...
		window.KE_INIT_FLAG = typeof window.KE_INIT_FLAG == 'undefined' ? 0 :  window.KE_INIT_FLAG+1;
		if(window.KE_INIT_FLAG > 0)KE.history.add(id, KE.g[id].minChangeSize);
	});
	if (KE.browser.IE) {
...
KE.plugin.cut = {
	click : function(id) {
		try {
		    alert('您的浏览器安全设置不允许编辑器自动执行剪切操作, 请使用键盘快捷键(Ctrl/Cmd+X)来完成');return;
...
KE.plugin.copy = {
	click : function(id) {
		try {
		    alert('您的浏览器安全设置不允许编辑器自动执行复制操作, 请使用键盘快捷键(Ctrl/Cmd+C)来完成');return;
...
KE.plugin.fontname = {
	...
	select: function(id , node){
	    var key = node.style.fontFamily;
		this.changeText(id , key);
	},
	exec : function(id, value) {
	    this.changeText(id , value);
		var cmd = new KE.cmd(id);
	...
	changeText : function(id , key){
	    var cmdName = 'fontname';
		if (KE.g[id].toolbarIcon[cmdName]) {
			var span = KE.g[id].toolbarIcon[cmdName][1];
			span = span.getElementsByClassName('ke-v')[0];
			if(key && typeof KE.lang.plugins.fontname.fontName[key] != 'undefined'){
			    span.innerHTML = KE.lang.plugins.fontname.fontName[key];
				span.className = 'ke-v v';
			}else{
			    span.innerHTML = '微软雅黑';
				span.className = 'ke-v v';
			}
...
KE.plugin.fontsize = {
    statck : ['9px', '10px', '12px', '14px', '16px', '18px', '24px', '32px'],
	click : function(id) {
		var fontSize = this.statck;
		var cmd = 'fontsize';
		KE.util.selection(id);
...
KE.plugin.link = {
	...
	click : function(id) {
	    var g = KE.g[id];
		var range = g.keRange;
		var p = range.startNode;
		var cmd = new KE.cmd(id);
		if(!p.tagName)p = g.keRange.getParentElement();
		if(p.tagName.toLocaleLowerCase() == 'data'){
		    return false;
		}else{
		    p = cmd.getTopParent(['data'] , p);
			if(p)return false;
		}
	...
	exec : function(id) {
		...
		var linkText = KE.$('linkText', dialogDoc).value;
		...
		if (isEmpty || KE.util.isEmpty(id)) {
			...
			arr[0].innerHTML = arr[0].innerHTML.replace(new RegExp('<a[^>]*>|<\/a[^>]*>') , '');
...
KE.plugin.advtable = {
		...
		var dialog = new KE.dialog({
			...
			afterShow : function(id){
				$(dialog.div).click(function(){
					var iframe = $('iframe' , dialog.div).get(0);
					if(typeof iframe !='undefined')iframe.contentWindow.hidePicker();
				});
				$(".ke-mask").click(function(){
				    var iframe = $('iframe' , dialog.div).get(0);
					if(typeof iframe !='undefined')iframe.contentWindow.hidePicker();
				});
			},
			yesClickFn : function(id) {
				var dialogDoc = KE.util.getIframeDoc(dialog.iframe),
					...
					owidthBox = KE.$('owidth', dialogDoc),
					oheightBox = KE.$('oheight', dialogDoc),
					...
					owidth = owidthBox.value ? parseInt(owidthBox.value) : 0,
					oheight = oheightBox.value ? parseInt(oheightBox.value) : 0,
					widthType = 'px',
					heightType = 'px',
				...
				if(width){
					...
					if(width != owidth){
						...
						var nextCell = cell.parentNode.cells[cell.cellIndex + 1];
						if(!!nextCell && nextCell.length){
							if(width > (owidth + nextCell.offsetWidth - 30))width = owidth + nextCell.offsetWidth - 30;
						}
						var offsetx = width - owidth;
						self.changeColWidth(cell , offsetx);
					}
				}
				...
				if(height){
					...
					if(height != oheight){
						...
						var offsety = height - oheight;
						self.changeRowHeight(cell , offsety);
					}
				}
	...
	tabledelete : function(id) {
		...
		if(!!this.onTableDelete)this.onTableDelete(table);
	...
	tablecolinsert : function(id, offset) {
		...
		var width = 0 , _cell = null , newCell = null;
		for (var i = 0, len = table.rows.length; i < len; i++) {
		    _cell = table.rows[i].cells[cell.cellIndex];
			if(_cell.colSpan > 1){
			    _cell.colSpan += 1; 
				continue;
			}

			newCell = table.rows[i].insertCell(index);
			newCell.innerHTML = '<br/>';
			width = cell.offsetWidth / 2;
			newCell.setAttribute("width" , width);
			_cell.setAttribute("width" , width);
		}
		this.resizeTableByTd(cell);
	},
	...
	tablecoldelete : function(id) {
		...
		if(cell.colSpan > 1){
			...
			var newCell = row.insertCell(index + 1);
				newCell.innerHTML = '<br/>';
				newCell.setAttribute("rowSpan" , cell.rowSpan);
				newCell.setAttribute("width" , newCell.offsetWidth);
		}else{
			for (var i = 0, len = table.rows.length; i < len; i++) {
				if(!!this.onTableDelete)this.onTableDelete(table.rows[i].cells[index]);
	...
	tablerowdelete : function(id) {
		...
		if(!!this.onTableDelete)this.onTableDelete(row);
		if(cell.rowSpan > 1){
		    cell.setAttribute('rowSpan' , cell.rowSpan - 1);
			var nextRow = table.rows[row.rowIndex + 1];
			var bCellIndex = cell.cellIndex;
			
			for(var i = 0; i < cell.colSpan ; i++){
				var newCell = nextRow.insertCell(bCellIndex + i);
				newCell.innerHTML = '<br/>';
				newCell.setAttribute("width" , newCell.offsetWidth);
			}
		}else{
	...
	tablerowmerge : function(id) {
		...
		// 最后一行不能合并
		if (table.rows.length <= nextRowIndex)return;
		
		var cells = this.getSameEndPosCells(cell , 'x' , true);
		if(cells.length < 2)return;
		var nextCell = null;
		for(var i in cells){
		    i = parseInt(i);
		    if(cells[i].parentNode.rowIndex == rowIndex){
			   nextCell = cells[i + 1];
			}
		}
		if(nextCell.parentNode.rowIndex - rowIndex - (cell.rowSpan - 1) != 1)return;

		// 上下行的colspan不一致时不能合并
		if (cell.colSpan !== nextCell.colSpan)return;
		cell.rowSpan += nextCell.rowSpan;
		cell.removeAttribute("height");
		if(!nextCell.innerHTML.match(new RegExp("^<[^>]*br>$")))cell.innerHTML += nextCell.innerHTML;
		nextRow.deleteCell(nextCell.cellIndex);
	...
	init : function(id) {
		...
		var tableCmds = 'prop,cellprop,colinsertleft,colinsertright,rowinsertabove,rowinsertbelow,rowmerge,colmerge,rowsplit,colsplit,coldelete,rowdelete,delete'.split(',');
		...
		this.initDragEvent(id);
	},
	click : function(id, mode) {
		mode = mode || 'default';
		var cmd = 'advtable',_self = this;
		KE.util.selection(id);
		this.dialog = new KE.dialog({
			id : id,
			cmd : cmd,
			...
			noButton : KE.lang['no'],
			afterShow : function(id){
			    var cell = _self.getSelectedCell(id);
			    _self.resizeTableByTd(cell);
				
				$(_self.dialog.div).click(function(){
					var iframe = $('iframe' , _self.dialog.div).get(0);
					if(typeof iframe !='undefined')iframe.contentWindow.hidePicker();
				});
				$(".ke-mask").click(function(){
				    var iframe = $('iframe' , _self.dialog.div).get(0);
	...
	exec : function(id) {
		...
		if (modeBox.value === 'update') {
			...
			if(borderColor == ''){border = 0;KE.addClass(table, zeroborder);}
			...
			if (!height && !!this.advtableRowHeight)height = rows * this.advtableRowHeight;
			if(height)style += 'height:' + height + heightType + ';';
			if (backgroundColor !== '') style += 'background-color:' + backgroundColor + ';';
			var html = '<table';
			if (style !== '') html += ' style="' + style + 'display:table;table-layout: fixed;"';
			if (padding !== '') html += ' cellpadding="' + padding + '"';
			if (spacing !== '') html += ' cellspacing="' + spacing + '"';
			if (align !== '') html += ' align="' + align + '"';
			if (border === '' || border === '0' || borderColor === '') html += ' class="' + zeroborder + '"';
			if (border !== '' && borderColor !== '') html += ' border="' + border + '"';
			if (borderColor !== '') html += ' bordercolor="' + borderColor + '"';
			html += '>';
			
			var tdWidth = Math.floor(width / cols - padding * 2 - border);
			
			for (var i = 0; i < rows; i++) {
				html += '<tr>';
				for (var j = 0; j < cols; j++) {
					html += '<td width="'+tdWidth+'"><br/></td>';
				}
				html += '</tr>';
			}
			html += '</table><p><br></p>';
			KE.util.insertHtml(id, html);
			//KE.util.execCommand(id , 'enableObjectResizing' , false);
			KE.util.execCommand(id, 'enableInlineTableEditing', false);//禁用行编辑
		}
		this.dialog.hide();
		KE.util.focus(id);
	},
	initDragEvent : function(id){
	    this.mouseDown = false;
		this.onDrag = '';
		this.dragTd = null;
	    var g = KE.g[id] , _self= this;
		
		KE.event.add(g.iframeDoc, 'mouseup', function(){
		    if(_self.onDrag){
			    var dragTdPos = KE.util.getElementPos(_self.dragTd),
                dragLinePos = KE.util.getElementPos(_self.dragLine);
			    switch (_self.onDrag) {
					case "h":
						_self.changeColWidth(_self.dragTd, dragLinePos.x - dragTdPos.x - _self.dragTd.offsetWidth);
						break;
					case "v":
						_self.changeRowHeight(_self.dragTd, dragLinePos.y - dragTdPos.y - _self.dragTd.offsetHeight);
						break;
					default:
				}
				_self.dragTd = null;
				_self.onDrag = '';
				_self.toggleDragableState(g.iframeDoc , false , '' , null);
			}
			_self.mouseDown = false;
		});
		
		KE.event.add(g.iframeDoc, 'mousemove', function(evt){
		    var target = _self.getParentTdOrTh(evt.target || evt.srcElement , g.iframeDoc);
			if(_self.onDrag && _self.dragTd){
			    $("body" , g.iframeDoc).css('webkitUserSelect' ,  'none');
				g.sel.removeAllRanges();//清除选区
                if (_self.onDrag == "h") {
                    _self.dragLine.style.left = _self.getPermissionX(_self.dragTd, evt) + "px";
                } else if (_self.onDrag == "v") {
				    var scTop = g.iframeDoc.body.scrollTop;
                    _self.dragLine.style.top = (_self.getPermissionY(_self.dragTd, evt) + scTop) + "px";
                }
                return;
			}
			
			if(target){
			    var dir = _self.getRelation(target  , KE.util.getCoords(evt));
			    _self.toggleDragableState(g.iframeDoc , true , dir , target);
			}
		});
		
		KE.event.add(g.iframeDoc, 'mousedown', function(evt){
		    var target = _self.getParentTdOrTh(evt.target || evt.srcElement , g.iframeDoc);
			if(target){
			     _self.dragTd = target;
		         _self.mouseDown = true;
			     _self.onDrag = _self.getRelation(target  , KE.util.getCoords(evt));
			}else{
			     $("body" , g.iframeDoc).css('cursor' , "text");
			}
		});
		
		KE.event.add(g.iframeDoc, 'mouseover', function(evt){
		    var target = _self.getParentTdOrTh(evt.target || evt.srcElement , g.iframeDoc);
			if(target && target.tagName == 'TD' && _self.mouseDown == false){
			    var dir = _self.getRelation(target  , KE.util.getCoords(evt));
			    _self.toggleDragableState(g.iframeDoc , true , dir , target);
			}
		});
		
		KE.event.add(g.iframeDoc, 'mouseout', function(evt){
		    var target = evt.target || evt.srcElement;
			$("body" , g.iframeDoc).css('cursor' , "text");
		});
		
		KE.onchange(id , function(){
		    _self.dragTd = null;
		    _self.onDrag = '';
			_self.mouseDown = false;
			_self.hideDragLine(g.iframeDoc);
			
			var table = $("table" , g.iframeDoc);
			if(table.attr('width') > 0){
			    table.css({width : table.width(), height : table.height()}).removeAttr("width height");
			}
		});
		
		KE.event.add(g.iframeDoc , "selectionchange", function () {console.log(1);
            _self.toggleDragableState(g.iframeDoc , false , '' , null);
        });
	...
	toggleDragableState : function(doc , status , dir , cell){
	    $("body" , doc).css('cursor' , dir == "h" ? "col-resize" : dir == "v" ? "row-resize" : "text");
	    if(status && dir && !this.mouseDown){
		     this.getDragLine(doc);
			 this.showDragLineAt(dir , cell , doc);
		}else{
			 this.hideDragLine(doc);
		}
	},
	/**
     * 获取鼠标与当前单元格的相对位置
    ...
    getRelation  : function(ele, mousePos) {
        var elePos = KE.util.getElementPos(ele);
	...
	/**
     * 依据dir（v|h）在cell位置显示横线
     * @param dir
     * @param cell
     */
    showDragLineAt : function(dir, cell , doc) {
        if (!cell) return;
        var table = $(cell).closest("table").get(0),
            width = table.offsetWidth,
            height = table.offsetHeight,
            tablePos = KE.util.getElementPos(table),
            cellPos = KE.util.getElementPos(cell), css = '',
			scTop = doc.body.scrollTop;
        switch (dir) {
            case "h":
                css = 'height:' + height + 'px;top:' + (tablePos.y + scTop) + 'px;left:' + (cellPos.x + cell.offsetWidth - 1);
                this.dragLine.style.cssText = css + 'px;position: absolute;display:block;background-color:#CCC;width:1px;border:0; color:#CCC;opacity:0.8;filter:alpha(opacity=80)';
                break;
            case "v":
                css = 'width:' + width + 'px;left:' + tablePos.x + 'px;top:' + (cellPos.y + scTop+ cell.offsetHeight - 1 );
                //必须加上border:0和color:blue，否则低版ie不支持背景色显示
                this.dragLine.style.cssText = css + 'px;overflow:hidden;position: absolute;display:block;background-color:#CCC;height:1px;border:0;color:#CCC;opacity:0.8;filter:alpha(opacity=80)';
                break;
            default:
        }
    },
	//创建拖动线
	getDragLine : function(doc) {
	    if (this.mousedown)return;
        this.dragLine = document.createElement("div");
        this.setAttributes(this.dragLine, {
            'class' : "ke_tableDragLine",
            unselectable:'on',
            contenteditable:false,
            'onresizestart':'return false',
            'ondragstart':'return false',
            'onselectstart':'return false',
            style:"background-color:#CCC;position:absolute;padding:0;margin:0;background-image:none;border:0px none;opacity:0;filter:alpha(opacity=0)"
        });
		$("body" , doc).get(0).appendChild(this.dragLine);
    },
	hideDragLine : function(doc){
	    if (this.mousedown)return;
	    $(".ke_tableDragLine" , doc).remove();
	},
	...
	getParentTdOrTh : function(ele , doc) {
        if (ele.tagName == "TD" || ele.tagName == "TH") return ele;
        var td;
		td = $(ele).closest('td');
		if(td.length)return td.get(0);
        return null;
    },
	getPermissionY : function(dragTd, evt) {
        var top = KE.util.getElementPos(dragTd).y,
            mousePosY = KE.util.getCoords(evt).y;
        return mousePosY < top ? top : mousePosY;
    },
	getPermissionX : function(dragTd, evt) {
        var table = $(dragTd).closest('table').get(0);
		var preTd = this.getSameEndPosCells(dragTd, "x")[0],
			nextTd = this.getSameStartPosXCells(dragTd)[0],
			mouseX = KE.util.getCoords(evt).x,
			left = (preTd ? KE.util.getElementPos(preTd).x : KE.util.getElementPos(table).x) + 20 ,
			right = nextTd ? KE.util.getElementPos(nextTd).x + nextTd.offsetWidth - 20 : (document.body.offsetWidth + 5 || parseInt(KE.getComputedStyle(document.body, "width"), 10));
		var x = mouseX < left ? left : mouseX > right ? right : mouseX;
		if(!nextTd){//计算是否大于表格的最大宽度
		    var maxW = KE.getComputedStyle(table , "max-width");
			if(maxW){
			   maxW = parseInt(maxW , 10) + 5 , t_x = KE.util.getElementPos(table).x;
			   if(x - t_x > maxW){
			       x = t_x + maxW;
			   }
			}
	...
	changeColWidth : function(td , changeValue){
	    if (Math.abs(changeValue) < 10) return;
        function _getWidth(cell){
		    if (!cell)return 0;
            return parseInt(KE.getComputedStyle(cell, "width"), 10);
        }
		
		function _getDefaultValue(_table){
		    var borderMap = {
                thin:'0px',
                medium:'1px',
                thick:'2px'
            },
			_td,tableBorder, tdPadding, tdBorder, tmpValue;
		    _td = _table.getElementsByTagName('td')[0];
            tmpValue = KE.getComputedStyle(_table, 'border-left-width');
            tableBorder = parseInt(borderMap[tmpValue] || tmpValue, 10);
            tmpValue = KE.getComputedStyle(_td, 'padding-left');
            tdPadding = parseInt(borderMap[tmpValue] || tmpValue, 10);
            tmpValue = KE.getComputedStyle(_td, 'border-left-width');
		...
		var table = $(td).closest('table').get(0),
			backTableWidth = _getWidth(table),
			defaultValue = _getDefaultValue(table),
		    //这里不考虑一个都没有情况，如果一个都没有，可以认为该表格的结构可以精简
			leftCells = this.getSameEndPosCells(td, "x"),
			backLeftWidth = _getWidth(leftCells[0]) - defaultValue.tdPadding * 2 - defaultValue.tdBorder,
			rightCells = this.getSameStartPosXCells(td),
			backRightWidth = _getWidth(rightCells[0]) - defaultValue.tdPadding * 2 - defaultValue.tdBorder;

		//整列被rowspan时存在
		var cell = null;
		for(var i in leftCells){
		    cell = leftCells[i];
		...
		this.resizeTableByTd(td);
	},
	changeRowHeight: function(td , changeValue){
	    if (Math.abs(changeValue) < 10) return;
		var cells = this.getSameEndPosCells(td, "y"),
		//备份需要连带变化的td的原始高度，否则后期无法获取正确的值
			backHeight = cells[0] ? cells[0].offsetHeight : 0;
		for (var i = 0, cell; cell = cells[i++];) {
			this.setCellHeight(cell, changeValue, backHeight);
		}
		this.resizeTableByTd(td);
	},
	setCellHeight : function (cell, height, backHeight) {
        var lineHight = parseInt(KE.getComputedStyle(cell, "line-height"), 10),
	 ...
	 */
	getSameEndPosCells: function (cell, xOrY , type) {
	    var table = $(cell).closest('table').get(0),
		    flag = (xOrY.toLowerCase() === "x"),
			end = KE.util.getElementPos(cell)[flag ? 'x' : 'y'] + cell["offset" + (flag ? 'Width' : 'Height')],
			rows = table.rows,
			cells = null, returns = [],num = rows.length;
		for (var i = 0; i < num; i++) {
			cells = rows[i].cells;
			for (var j = 0, tmpCell; tmpCell = cells[j++];) {
				var tmpEnd = KE.util.getElementPos(tmpCell)[flag ? 'x' : 'y'] + tmpCell["offset" + (flag ? 'Width' : 'Height')];
				...
				if (cell == tmpCell || end == tmpEnd) {
					...
					if(!type){ //普通的校验，只或单一单元格
						if (tmpCell[flag ? "colSpan" : "rowSpan"] == 1) {
							returns.push(tmpCell);
						}
					}else{ //获取所有(宽度/高度)和结束位置一致的单元格
					    if(tmpCell["offset" + (flag ? 'Width' : 'Height')] == cell["offset" + (flag ? 'Width' : 'Height')]){
						    returns.push(tmpCell);
						}
					}
					if (flag) break;
	...
	getSameStartPosXCells:function (cell) {
	    var table = $(cell).closest('table').get(0),
		    start = KE.util.getElementPos(cell).x + cell.offsetWidth,
			rows = table.rows, cells , returns = [] , num = rows.length;
		for (var i = 0; i < num; i++) {
			cells = rows[i].cells;
			for (var j = 0, tmpCell; tmpCell = cells[j++];) {
				var tmpStart = KE.util.getElementPos(tmpCell).x;
				if (tmpStart > start + 5) break;
				if ((tmpStart == start || (tmpStart - start < 5 && tmpStart - start > -5)) && tmpCell.colSpan == 1) {
					returns.push(tmpCell);
					break;
				}
			}
	...
	resizeTableByTd : function(cell){
	    var table = $(cell).closest('table');
		table.css({width: table.width() , height: table.height()});
		table.removeAttr("width");
		table.removeAttr("height");
	},
	resizeTd : function(table){
	    var rows = table.rows;
		for(var i in rows){
			for(var j in rows[i].cells){
				if(rows[i].cells[j].colSpan == 1 && rows[i].cells[j].rowSpan == 1){
					rows[i].cells[j].removeAttribute("width");
					rows[i].cells[j].removeAttribute("height");
					//rows[i].cells[j].setAttribute('width' , rows[i].cells[j].offsetWidth);
				}
			}
		}
	}
